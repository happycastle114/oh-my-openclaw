...
> **Tip:** Run `openclaw omoc-setup` to inject agent tool profiles into your `openclaw.json5` for runtime enforcement.
...
> **Important:** These are prompt-level hints only. For runtime enforcement, run `openclaw omoc-setup` to inject agent configs into your `openclaw.json5`.
...
### Runtime Enforcement (omoc-setup CLI)

For hard enforcement at the OpenClaw runtime level, run:

```bash
openclaw omoc-setup
```

This injects all 11 agent configs (with model, tools, subagents) into your `openclaw.json5`. Use `--force` to overwrite existing configs, `--dry-run` to preview.
